...
export const Main: FC = memo(() => {
  ...
  const maxItemCollections = useAppSelector<Collection[]>(
    state => state.collection.maxItemCollections
  )

  return (
    <>
      <Divider plain>
        <span>{t('main.collectionsInfo')}</span>
      </Divider>
            ...
            return (
              <div key={el._id}>
                <Card
                  ...
                  cover={
                    <div className={s.imgWrap}>
                      {el.image !== null ? (
                        <Image width={250} height={150} src={`${el.image}`} />
                      ) : (
                        <ErrorImage />
                      )}
                    </div>
                ...
                >
                  <Meta
                    title={
                      <NavLink to={`${routes.COLLECTIONS}/${el.user._id}${routes.ITEMS}/${el._id}`}>
                        {el.name}
                      </NavLink>
                    }
                    description={el.theme}
                  />
                  <ReactMarkdown>{`${el.description.slice(0, 40)}...`}</ReactMarkdown>
            ...
            return (
              <div key={el._id}>
                ...
                >
                  ...
                  />
                  <p>
                    {t('main.likeCount')}: {el.likes.length}
                  </p>
                  <p>
                    {t('main.commentCount')}: {el.countComments}
                  </p>
                </Card>
              </div>
            )
